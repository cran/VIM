...
gowerD <- function(data.x, data.y = data.x, weights=NULL,numerical,factors,orders,mixed,levOrders) {
  maxplus1 <- function(x){
  ...
  out <- .Call( "gowerD", data.x, data.y,weights,c(length(numerical),length(factors),length(orders),length(mixed)),levOrders,PACKAGE="VIM")
  if(justone)
...
"gower.dist" <- function (data.x, data.y = data.x, rngs = NULL, KR.corr = TRUE,weights=NULL) 
{
  gower.fcn <- function(x, y, rng = NULL, KR.corr = TRUE, weights=NULL) {
    nx <- length(x)
    ny <- length(y)
    cx <- class(x)
    cy <- class(y)
